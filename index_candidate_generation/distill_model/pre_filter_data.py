...
from index_candidate_generation.distill_model.distill_utils.distill_const import ops_join_dict, ops_scan_dict, ops_sort_dict, ops_group_dict
...
def extract_ind_col(index):
    ...
    cols_in_ind = cols_in_ind[0]
    tab = cols_in_ind[0].split('.')[0]
    cols = []
...
def extract_feat(query, root, index, row, stats):
    ...
    for sc in scan:
        ...
        for col in index.columns:
            if col in str(sc) and index.table_name in str(sc):
    ...
    for jo in join:
        ...
        for col in index.columns:
            # print(col, index.table_name)
            if col in str(jo_str) and index.table_name in str(jo_str):
    ...
    index_shape = list()
    for col in index.columns:
        if col in str(sort_str) and index.table_name in str(sort_str):
            index_shape.append("order-by")
        elif col in str(group_str) and index.table_name in str(group_str):
            index_shape.append("group-by")
        elif col in str(join_str) and index.table_name in str(join_str):
            index_shape.append("join")
        elif col in str(scan_str) and index.table_name in str(scan_str):
            index_shape.append("selection")

    if len(index_shape) == 1 and index_shape[0] in ["order-by", "group-by"]:
        for col in index.columns:
            if col in str(join_str) and index.table_name in str(join_str):
                index_shape = ["join"]
                break
            elif col in str(scan_str) and index.table_name in str(scan_str):
    ...
    for so in sort:
        ...
        for col in index.columns:
            if col in str(so_str) and index.table_name in str(so_str):
                # print(col)
                density = stats[f"{index.table_name}.{col}"]["dist"]
    ...
    for gr in group:
        ...
        for col in index.columns:
            if col in str(gr_str) and index.table_name in str(gr_str):
                density = stats[f"{index.table_name}.{col}"]["dist"]
...
# lzdh
def get_utility(root, index, row):
    # 1. tag the node in the plan
    seq = list()
    terminal = list()
    traverse_plan(seq, terminal, root, None)
...
def gen_feat_batch(type='infer'):
    ...
    schema_dict = {}
    for bench in tqdm(benchmarks):
        stats_load = f"../../data/dbstats/db_stats_{bench}.json"
        with open(stats_load, "r") as rf:
            stats = json.load(rf)
        stats_dict[bench] = stats

        schema_load = f"../../data/schemas/{bench}.json"
        with open(schema_load, "r") as rf:
            schema = json.load(rf)
        row = dict()
        for item in schema:
            row[item["table"]] = item["rows"]
        schema_dict[bench] = row

    feat_data = list()

    data_load = f"../../../plan_raw_{type}.json"
    ...
    for no, dat in enumerate(data):
        ...
        index = dat["indexes"]
        # print(index)
        table, columns = extract_ind_col(index)
        index = Index(columns, table)
        # print(index.columns)
        feat = extract_feat(query, root, index, schema_dict[b], stats_dict[b])
        dat["feat"] = feat
        feat_data.append(dat)

    data_save = f"../data/plan_raw_feat_{type}.json"
    with open(data_save, "w") as wf:
